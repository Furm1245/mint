...
contract RoboDucksNFT is ERC721, Ownable, ReentrancyGuard, PaymentSplitter {
    ...
    function presaleMint(
        ...
        require(msg.sender == account, "RoboDucksNFT: Not allowed");
        require(presaleM, "RoboDucksNFT: Presale is OFF");
        require(!paused, "RoboDucksNFT: Contract is paused");
        require(
            _amount <= presaleAmountLimit,
            "RoboDucksNFT: You can't mint so much tokens"
        );
        require(
        ...
        require(
            ...
            "RoboDucksNFT: max supply exceeded"
        );
        require(
            _price * _amount <= msg.value,
            "RoboDucksNFT: Not enough ethers sent"
        );
    ...
    function publicSaleMint(uint256 _amount) external payable onlyAccounts {
        require(publicM, "RoboDucksNFT: PublicSale is OFF");
        require(!paused, "RoboDucksNFT: Contract is paused");
        require(_amount > 0, "RoboDucksNFT: zero amount");
